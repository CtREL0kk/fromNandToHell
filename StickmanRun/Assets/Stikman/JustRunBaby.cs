...
public class Keyboard : MonoBehaviour
{
    ...
    private void HandleObstacle()
    {
        if (whiteSquareInstance == null)
        {
            ...
            whiteSquareInstance.transform.position = whiteSquareInstance.transform.position + new Vector3(0, 0, -2); //на передний план
            moveSpeed = 0;
            StartShakeCamera();
    ...
    private void Fall(RaycastHit2D wallHit)
    {
        ...
        if (boxCollider != null && transform.position.y < boxCollider.bounds.size.y)
        {
            ...
            moveSpeed = 0;
            DisableHead();//чувак головой цепляется за платформу
        }
    }
